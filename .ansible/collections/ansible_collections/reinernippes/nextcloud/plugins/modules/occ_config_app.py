...
DOCUMENTATION = r'''
---
module: occ_config_app
short_description: Manage Nextcloud app config via occ
version_added: "1.0.0"
description:
  - Idempotent module for setting and deleting Nextcloud app configuration values via C(occ).
...
options:
  ...
  app:
    description:
      - App name whose config to manage.
    required: true
    type: str
  key:
    description:
      - Config key to set or delete.
    required: true
    type: str
  value:
    description:
  ...
  value_type:
    description:
      - Nextcloud config type to pass to C(config:app:set).
    type: str
    default: string
    choices:
      - string
...
EXAMPLES = r'''
- name: Configure OnlyOffice server URL
...
def get_config_state(module, params):
    argv = ["config:app:get", str(params["app"]), str(params["key"])]
...
def main():
    ...
    if state == "absent":
        if not current["exists"]:
            module.exit_json(**result)
        cmd = ["config:app:delete", str(params["app"]), str(params["key"])]
    ...
    cmd = ["config:app:set", str(params["app"]), str(params["key"])]
